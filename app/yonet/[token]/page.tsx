...
export default async function Yonet({ params, searchParams }: { params: Promise<{ token: string }>; searchParams: Promise<{ guncellendi?: string; bolum?: string }> }) {
  ...
  return <Workspace title={`${inv.name_a} ile ${inv.name_b}`} subtitle={`${shortDate(inv.main_date)}${inv.city ? ` · ${inv.city}` : ""}`} label="Çiftin yönetim alanı"
    ...
    {active === "davetiye" && <div className="invitation-workspace"><div><div className="yonet-kapak" style={themeScope(inv.theme)}><Hero inv={inv} /></div><p className="muted small centered">{themeOf(inv.theme).label} · {ornamentOf(inv.ornament).label} · {fontOf(inv.font).label} isimler</p></div><div className="stack"><section className="surface"><p className="eyebrow">Sizin davetiyeniz</p><h2>Her ayrıntı yerli yerinde.</h2><p className="muted">İsimleri, tarihleri, mekânı veya temayı dilediğiniz zaman güncelleyin. Davetli linkleri aynı kalır.</p><Link className="btn full" href={`${root}/duzenle`}>Davetiyeyi düzenle</Link><Link className="btn ghost full spaced" href={`/onizleme/${token}`}>Önizle ve hikâye görselini indir</Link></section><section className="surface qr-kart"><p className="eyebrow">Basılı davetiye</p><h2>QR kod</h2><img src={`${root}/qr?bicim=svg`} alt="Davetiye QR kodu" width={132} height={132} /><p className="muted">Kâğıt davetiyenize ekleyin. Okutan kişi davetiyeyi, yol tarifini ve takvime ekle düğmesini görür; davetli listesi ve kişisel yanıtlar görünmez.</p><a className="btn full" href={`${root}/qr`} download>QR kodu indir (PNG)</a><a className="lnk spaced" href={`${root}/qr?bicim=svg`} download="buyrun-davetiye-qr.svg" style={{ display: "inline-block" }}>Matbaa için vektörel (SVG)</a><p className="muted small">Baskıda en az 2,5 cm olsun; açık zemin üzerine koyun.</p></section><section className="surface"><h2>Paylaşmaya hazır</h2><p className="muted">Kişiye özel davetleri aile panelinden hazırlayın. WhatsApp önizleme posteri seçtiğiniz temayı kullanır.</p><Link className="lnk" href={`${root}?bolum=aileler`}>Aile panellerine git →</Link></section></div></div>}
