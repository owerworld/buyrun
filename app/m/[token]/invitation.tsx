...
export default function Invitation({event,inviteToken,initialGuestToken}: {
  event:InvitationEvent;inviteToken:string;initialGuestToken:string|null
}) {
  ...
  async function submit(e:React.FormEvent) {
    ...
    if(status==="pending"){setError("Önce katılım durumunu seç.");return;}
    setBusy(true);
    try {
  ...
  return <main className={styles.page}>
    <div className={styles.shell}>
      ...
      <section id="invitation" className={styles.details}>
        <div className={styles.host}><span className={styles.avatar}>{event.hostName.slice(0,1).toLocaleUpperCase("tr")}</span><div><span>EV SAHİBİ</span><strong>{event.hostName}</strong></div><span className={styles.star}>✳</span></div>
        <div className={styles.infoRow}><span className={styles.infoIcon}>↗</span><div><strong>{dateLabel}</strong><p>Saat {event.time}</p></div></div>
        <div className={styles.infoRow}><span className={styles.infoIcon}>⌖</span><div><strong>{event.venue}</strong>{event.address&&<p>{event.address}</p>}<a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent([event.venue,event.address].filter(Boolean).join(" "))}`} target="_blank" rel="noreferrer noopener">Haritada aç ↗</a></div></div>
        {event.description&&<div className={styles.description}><h2>Bir araya gelelim.</h2><p>{event.description}</p></div>}
      </section>
      <section className={styles.rsvp} aria-labelledby="rsvp-title">
        <div className={styles.formHeading}><span className={styles.eyebrow}>YERİN HAZIR</span><h2 id="rsvp-title">Sen de geliyor musun?</h2><p>Ev sahibine haber ver, planlar tamamlansın.</p></div>
        <form onSubmit={submit}>
          <fieldset disabled={busy} className={styles.fieldset}>
            ...
            <label className={styles.label}>Adın soyadın<input value={name} onChange={e=>{setName(e.target.value);setSaved(false);}} placeholder="Sana nasıl seslenelim?" autoComplete="name" required maxLength={100}/></label>
            {status!=="declined"&&<div className={styles.countRow}><div><strong>Kaç kişi geliyorsunuz?</strong><p>Kendin dahil toplam kişi sayısı</p></div><div className={styles.stepper}><button type="button" disabled={count<=1} onClick={()=>{setCount(c=>Math.max(1,c-1));setSaved(false);}} aria-label="Kişi sayısını azalt">−</button><output aria-live="polite">{count}</output><button type="button" disabled={count>=20} onClick={()=>{setCount(c=>Math.min(20,c+1));setSaved(false);}} aria-label="Kişi sayısını artır">+</button></div></div>}
            <label className={styles.label}>Ev sahibine not <span>isteğe bağlı</span><textarea value={note} onChange={e=>{setNote(e.target.value);setSaved(false);}} maxLength={500} placeholder="Bir şey getireyim mi?" rows={3}/></label>
